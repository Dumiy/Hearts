...
class playGround():
    ...
    def cardsPile(self,pile):
        for x in pile:
            self.pile.append(x)
    def play_obo(self,players): #drawing cards obo - one by one or 5pp - 5 playing cards per player
        for _ in range(5):
...
class Player(Deck,playGround):
    hand = list()
    score = int()
    def __init__ (self,hand=list(),score = 0,deck = Deck()):
        self.hand = hand
        self.score = score
